...
function Projectile:_move()
    self.x = self.x + self.direction_x*self.velocity
    self.y = self.y + self.direction_y*self.velocity
    self.life = self.life - 1
end

function Projectile._insert_projectile(enemy, bullets)
    ratio = enemy.ratio
    ratio_max = enemy.max_ratio
    if ratio == ratio_max then
        enemy.ratio = 0
        table.insert(bullets, enemy:_shoot())
    end
end

function Projectile:_kill_projectile(bullets, i)
